...
@section('title', __('ui.hariri_bidhaa'))
@section('page-title', __('ui.hariri_bidhaa') . ': ' . $bidhaa->jina)

@section('content')
<div class="py-4 max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('bidhaa.update', $bidhaa) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.jina') }} *</label>
                <input type="text" name="jina" value="{{ old('jina', $bidhaa->jina) }}" required
            ...
            <div>
                ...
                <input type="text" name="kategoria" value="{{ old('kategoria', $bidhaa->kategoria) }}"
                       list="kategoria-list"
                       class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"/>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="bei_halisi" value="{{ old('bei_halisi', $bidhaa->bei_halisi) }}" required min="0" step="0.01"
                           id="bei_halisi" oninput="hitesabu()"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"/>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.bei_yangu') }} (TZS) *</label>
                    <input type="number" name="bei_yangu" value="{{ old('bei_yangu', $bidhaa->bei_yangu) }}" required min="0" step="0.01"
                           id="bei_yangu" oninput="hitesabu()"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"/>
                </div>
            </div>

            <div id="faida-preview" class="bg-green-50 border border-green-200 rounded-lg px-4 py-3 text-sm">
                <div class="flex justify-between">
                    <span class="text-gray-600">{{ __('ui.faida') }}:</span>
                    <span id="faida-thamani" class="font-bold text-green-700">{{ number_format($bidhaa->faida) }} TZS</span>
                </div>
                <div class="flex justify-between mt-1">
                    <span class="text-gray-600">{{ __('ui.faida_asilimia') }}:</span>
                    <span id="faida-asilimia" class="font-bold text-green-700">{{ $bidhaa->faida_asilimia }}%</span>
                </div>
            </div>
            ...
            <div>
                ...
                <select name="hali" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400">
                    <option value="inapatikana" {{ old('hali', $bidhaa->hali) === 'inapatikana' ? 'selected' : '' }}>{{ __('ui.inapatikana') }}</option>
                    <option value="imeuzwa" {{ old('hali', $bidhaa->hali) === 'imeuzwa' ? 'selected' : '' }}>{{ __('ui.imeuzwa') }}</option>
                    <option value="imesimamishwa" {{ old('hali', $bidhaa->hali) === 'imesimamishwa' ? 'selected' : '' }}>{{ __('ui.imesimamishwa') }}</option>
                </select>
            </div>
            ...
            <div>
                ...
                <textarea name="maelezo" rows="3"
                          class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none">{{ old('maelezo', $bidhaa->maelezo) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.picha') }}</label>
                @if($bidhaa->picha)
                    <img src="{{ Storage::url($bidhaa->picha) }}" class="w-24 h-24 object-cover rounded-lg mb-2 border"/>
                @endif
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('bidhaa.show', $bidhaa) }}" class="flex-1 text-center border border-gray-200 text-gray-600 hover:bg-gray-50 py-2.5 rounded-lg text-sm font-medium transition">{{ __('ui.rudi') }}</a>
            </div>
        </form>
    </div>
</div>
