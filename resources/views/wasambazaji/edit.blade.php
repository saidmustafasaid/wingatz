...
@section('title', __('ui.hariri_msambazaji'))
@section('page-title', __('ui.hariri_msambazaji'))

@section('content')
<div class="py-4 max-w-xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('wasambazaji.update', $wasambazaji) }}" class="space-y-4">
            @csrf @method('PUT')

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.jina') }} *</label>
                <input type="text" name="jina" value="{{ old('jina', $wasambazaji->jina) }}"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="simu" value="{{ old('simu', $wasambazaji->simu) }}"
                ...
                <div>
                    ...
                    <input type="text" name="whatsapp" value="{{ old('whatsapp', $wasambazaji->whatsapp) }}"
                           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="bidhaa_wanazouza" value="{{ old('bidhaa_wanazouza', $wasambazaji->bidhaa_wanazouza) }}"
                       class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300">
            </div>
            ...
            <div>
                ...
                <textarea name="maelezo" rows="3"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300">{{ old('maelezo', $wasambazaji->maelezo) }}</textarea>
            </div>

            <div class="flex gap-3 pt-2">
                <button type="submit" class="btn-primary text-white px-6 py-2.5 rounded-xl text-sm font-semibold">{{ __('ui.hifadhi') }}</button>
                <a href="{{ route('wasambazaji.show', $wasambazaji) }}" class="bg-gray-100 text-gray-700 px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-200 transition">{{ __('ui.rudi') }}</a>
            </div>
        </form>
    </div>
</div>
